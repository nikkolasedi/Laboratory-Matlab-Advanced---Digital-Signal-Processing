...
% Excercise 1.1.4
figure
histogram(y,'BinEdges',m:1/500:M);
% before correction: histogram(y, 32, 'BinLimits', [-0.25 0.25])
...
subplot(1,3,1)
% before correction: histogram(r1, 32, 'BinLimits', [min(r1) max(r1)])
histogram(r1,'BinEdges',min(r1):1/500:max(r1));
title('Histogram for rand')

subplot(1,3,2)
%histogram(r2, 32, 'BinLimits', [min(r2) max(r2)])
histogram(r2,'BinEdges',min(r2):1/500:max(r2));
title('Histogram for randn')

subplot(1,3,3)
%histogram(r3, 32, 'BinLimits', [min(r3) max(r3)])
histogram(r3,'BinEdges',min(r3):1/500:max(r3));
...
q1(q1>=0.875) = 0.875;
q2(q2>=(1-1/32)) = 1-1/32;
q3(q3>=(1-1/128)) = 1-1/128;
% qy = 0.25 * round(yn/0.25);

% q1 = i1*round(ytest/i1);
% q1(q1>=0.875) = 0.875;
...
yn1 = y.*(0.01/max(abs(y)));%rescale to  [-0.01,0.01]
...
% ind = find(q4 == max(q4) | q4 == min(q4));
% ind = find(abs(yn1)>abs(q4))

% ind1 = find(q1(69000:69499)==0.75);
% ind2 = find(q2(69000:69499)==0.75);
% ind3 = find(q3(69000:69499)==0.75);
...
xlabel('No. of sample')
ylabel('Amplitude')
legend('Normalized Signal', 'Quantized Signal')
...
plot(t1, env1, 'b')
hold on
plot(t1, hilbert(s_AM1), 'r')
xlabel('Time in s')
ylabel('Amplitude')
legend('Complex Envelope', 'Analytical Signal')
title('Envelope & Analytical Signal')
