...
//     Calling waitbar Matlab function
    
//     char *functionName = "waitbar";
//     int nin = 2;
//     int nout = 1;
    mxArray *input[2];
    mxArray *output[1];
    input[0] = mxCreateDoubleScalar(0);
    input[1] = mxCreateString("Starting calculation");
    
    mexCallMATLAB(1, output, 2, input, "waitbar");
 
    mxArray* h = output[0];
//     mexEvalString("h = waitbar(0,'Initializig variables...');");
    ...
    /* allocate memory */
//     mexEvalString("waitbar(0.1, h,'Allocating memory...');");
...
//     mexEvalString("waitbar(%d, h,'starting calculations...');", (double)currentPeriod/numberPeriods);
    for (k = 0; k < (lengthInputSignal - period); k++) { 
        
        
        if(currentPeriod%100 == 0){
        
            input[0] = mxCreateDoubleScalar( (double)currentPeriod/numberPeriods);
            input[1] = h;
            input[2] = mxCreateString("Starting calculation");
            
         
            mexCallMATLAB(0, NULL, 3, input, "waitbar");
    ...
    mxArray *h1[1];
    h1[0] = h;
    mexCallMATLAB(0, NULL, 1, h1, "close");
//     mexEvalString("waitbar(1, h,'Ready');");
